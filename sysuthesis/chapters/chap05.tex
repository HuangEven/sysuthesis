...
图~\ref{fig:multi-gpu-principles} 汇总了多 GPU 方案的四项设计原则，表~\ref{tab:primary-vs-fallback} 对主方案与兜底方案的适用边界进行了归纳，图~\ref{fig:query-parallel-vs-cross} 则直观比较了两类并行路径。
...
图~\ref{fig:control-plane} 展示了 CPU 控制面的调度与回收流程，表~\ref{tab:control-vs-data-plane} 说明了控制面与数据平面的职责边界，图~\ref{fig:bounded-batching} 则刻画了 bounded batching 与动态负载均衡的工作方式。
