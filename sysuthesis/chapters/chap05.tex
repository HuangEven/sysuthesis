...
图~\ref{fig:need-multi-gpu} 展示了单卡能力边界如何随数据规模和并发需求逼近上限，表~\ref{tab:multi-gpu-needs} 对扩展动机进行了分类，图~\ref{fig:bottleneck-shift} 则说明瓶颈从单卡阶段向多卡聚合与共享 I/O 路径迁移的过程。
...
图~\ref{fig:multi-gpu-principles} 汇总了多 GPU 方案的四项设计原则，表~\ref{tab:primary-vs-fallback} 对 replicated 主方案、sharded+GPU 归并兜底方案以及 CPU 聚合对比路径的适用条件进行了归纳，图~\ref{fig:query-parallel-vs-cross} 则直观比较了两类并行路径。

\paragraph{图表预留说明：}
\begin{itemize}
  \item \textbf{图 5-3 多 GPU 扩展策略设计原则图。} 概括四项设计原则及其相互关系。
  \item \textbf{表 5-2 多 GPU 扩展方案适用条件表。} 列出 replicated、sharded+GPU 归并以及 CPU 聚合对比路径的适用边界。
...
图~\ref{fig:replicated-arch} 给出了 replicated 主方案的总体结构，表~\ref{tab:replicated-pros-cons} 概括了其资源代价与适用场景，图~\ref{fig:single-loop-replicated} 则强调单卡优化链路在多卡上的复用关系。
...
图~\ref{fig:control-plane} 展示了 CPU 控制面的调度与回收流程，表~\ref{tab:control-vs-data-plane} 汇总了控制面、数据平面、通信与 I/O 路径中的职责划分及关键参数，图~\ref{fig:bounded-batching} 则刻画了 bounded batching 与动态负载均衡的工作方式。

\paragraph{图表预留说明：}
\begin{itemize}
  \item \textbf{图 5-7 CPU 控制面调度流程图。} 展示请求队列、批次形成、GPU 负载监控、请求分发和结果回收路径。
  \item \textbf{表 5-4 控制面/数据面职责与关键参数表。} 汇总控制面、数据平面、通信与 I/O 相关组件的职责和关键参数。
...
图~\ref{fig:nccl-merge} 展示了 GPU 侧跨卡归并的主要流程，表~\ref{tab:merge-strategies} 总结了不同触发条件下的处理策略，图~\ref{fig:nccl-streams} 则说明 NCCL 通信流与计算流的组织差异。
...
图~\ref{fig:topology} 以左右对比方式展示了拓扑感知 I/O lane 在合理绑定与共享冲突两种条件下的 GPU、SSD、PCIe 与 NUMA 关系，图~\ref{fig:io-lane-binding} 说明了 I/O lane 绑定与路径冲突的差异，表~\ref{tab:io-lanes} 则整理了准入控制所使用的关键参数。

\paragraph{图表预留说明：}
\begin{itemize}
  \item \textbf{图 5-11 拓扑感知 I/O lane 示意图。}
...
图~\ref{fig:multi-gpu-runtime} 展示了多 GPU 运行时组织方式，表~\ref{tab:multi-gpu-engineering} 汇总了工程增强项，图~\ref{fig:multi-gpu-metrics} 则概括了运行期监控指标体系。
...
表~\ref{tab:multi-gpu-exp-setup} 给出了多 GPU 实验设置，图~\ref{fig:qps-vs-gpu} 与图~\ref{fig:latency-quantiles} 展示扩展曲线与延迟分位数变化，图~\ref{fig:cpu-vs-gpu-merge} 和图~\ref{fig:io-lane-impact} 分别用于比较归并路径和 I/O lane 配置的影响，表~\ref{tab:multi-gpu-summary} 对总体结果进行了汇总。
