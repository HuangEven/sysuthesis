...
本节从融合系统总体对比、单卡优化与多 GPU 扩展收益汇总，以及不同数据规模下的系统表现趋势三个层面展示实验结果。
...
对比第四章单卡优化与第五章多 GPU 扩展后的端到端表现：原始分段链路、完整单卡优化、2 GPU replicated 扩展以及 4 GPU replicated 扩展。重点体现单卡优化主要降低阶段边界开销与尾延迟，而多 GPU 扩展主要提升吞吐上限并进一步降低 CPU 压力\citep{faiss_gpu_wiki,rapids_rmm_pinned,xiao2025flashanns,cuvs_multi_gpu_nn}。
...
比较 1M、10M、50M、100M 四个规模层级下系统的瓶颈迁移与收益变化，统一呈现``方法有效性''与``系统可扩展性''两条主线。
