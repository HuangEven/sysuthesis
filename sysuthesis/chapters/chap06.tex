...
\begin{itemize}
  ...
  \item \textbf{图 6-4 融合系统与基线系统检测效果对比图。}
  \item \textbf{图 6-5 融合系统与基线系统吞吐对比图。}
  \item \textbf{图 6-6 融合系统效果与性能折中图。}
  \item \textbf{图 6-7 单卡优化与多 GPU 扩展收益图。}
  \item \textbf{图 6-8 不同规模下系统表现趋势图。}
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{2.5cm}p{2.2cm}p{3.0cm}p{5.2cm}p{2.0cm}}
    ...
    \hline
    融合架构总体有效性 & 完整融合 & PR-AUC, QPS & 在 PR-AUC 仅略低于全量评分的前提下，QPS 相比全量评分显著提升，说明``候选召回 + 张量化评分''能够兼顾效果与吞吐 & 图 6-4--图 6-6 \\
    单卡链路优化收益 & 单卡优化 & QPS, p99 Latency, CPU Utilization & GPU 主导执行链路显著压缩阶段边界开销，p99 延迟和 CPU 负载同步下降，端到端吞吐明显提升 & 图 4-14--图 4-16，图 6-7 \\
    多 GPU replicated 扩展性 & 2/4 GPU replicated & QPS, CPU Utilization & GPU 数量增加时系统吞吐近似线性增长，CPU 利用率未同步放大，说明控制面/数据平面分离设计有效 & 图 5-15，图 6-7 \\
    跨卡归并策略差异 & GPU merge vs.\ CPU aggregation & avg/p99 Latency & 当索引无法完整复制时，GPU 侧归并优于 CPU 聚合，说明``CPU 退出数据平面''在多 GPU 场景中仍然重要 & 图 5-16，表 5-2 \\
    跨规模可行性 & 完整融合 & QPS, p99 Latency, PR-AUC & 从 1M 扩展到 100M 时，QPS 随规模增大而下降、p99 上升，但 PR-AUC 基本稳定，且融合系统持续优于基线 & 图 6-8 \\
    \hline
  \end{tabular}
  \end{adjustbox}
\end{table}
...
图~\ref{fig:discussion-logic} 总结了本节讨论所遵循的因果链条，表~\ref{tab:findings-evidence} 则把关键发现与其对应实验依据进行了对应整理。

\paragraph{图表预留说明：}
\begin{itemize}
  \item \textbf{图 6-9 结果讨论逻辑图。}
  \item \textbf{表 6-8 核心结果与证据对应表。}
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{3.1cm}p{3.4cm}p{5.4cm}p{2.0cm}}
    ...
    \hline
    融合系统在接近最佳检测效果下显著提升吞吐 & 图 6-4，图 6-5，图 6-6，表 6-7 & 完整融合方案的 PR-AUC 接近全量评分，但 QPS 显著领先，证明两阶段融合路线在效果与性能之间取得更优平衡 & 6.4.1 \\
    单卡优化的核心收益来自数据通路压缩而非单一算子加速 & 图 4-14，图 4-16，图 6-7 & 查询组织、候选打包和结果回传阶段开销下降最明显，说明 GPU 主导链路实质上减少了 CPU$\leftrightarrow$GPU 往返与同步等待 & 4.8，6.4.2 \\
    多 GPU 主方案应优先采用 replicated 架构 & 表 5-2，图 5-15，图 6-7 & 在显存允许的前提下，replicated 方案扩展路径更简单、CPU 压力更低，吞吐随 GPU 数量增加保持稳定增长 & 5.3，5.8，6.4.2 \\
    必须跨卡归并时 GPU 侧归并优于 CPU 聚合 & 图 5-16，表 5-2 & GPU merge 的平均延迟与 p99 延迟均低于 CPU aggregation，说明大规模中间结果不宜回流主机侧处理 & 5.5，5.8 \\
    I/O 路径组织决定多 GPU 扩展上限 & 图 5-11，图 5-18，图 6-8 & 当多卡共享 PCIe/SSD 路径时，p99 延迟上升且 QPS 恶化，合理 lane 绑定是释放多 GPU 吞吐的必要条件 & 5.6，5.8，6.4.3 \\
    out-of-core 融合方案在超大规模下仍具工程可行性 & 图 6-8，表 6-7 & 即便扩展到 100M 规模，系统仍保持稳定的 PR-AUC，并在吞吐和尾延迟上继续优于基线，说明外存候选召回结合 GPU 评分具有可行性 & 6.4.3 \\
    \hline
  \end{tabular}
  \end{adjustbox}
\end{table}
...
表~\ref{tab:limitations} 汇总了主要局限性及其影响范围，图~\ref{fig:applicability-boundary} 则用边界示意图说明了本文结论更适用的场景范围。

\paragraph{图表预留说明：}
\begin{itemize}
  \item \textbf{表 6-9 研究局限性与影响范围表。}
  \item \textbf{图 6-10 本文结论适用边界示意图。}
\end{itemize}
