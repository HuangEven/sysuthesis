...
在金融大规模异常检测场景中，系统面对的已不再是单一检测模型的局部性能问题，而是一个包含候选检索、异常评分、数据路径组织、设备间协同与结果追踪在内的系统工程问题。尤其在连续到达、时延敏感和峰值流量显著的业务条件下，异常检测系统既要对少量高风险样本保持有效识别能力，又要在大规模数据条件下维持稳定吞吐与可接受延迟。因此，本文的研究对象不是某一种独立的异常检测算法，也不是某一个单独的向量检索组件，而是一个面向金融大规模异常检测任务的融合系统。

从处理流程上看，本文关注的系统链路可概括为：对原始金融交易记录、账户行为统计和上下文特征进行规范化与向量化表示；基于向量化表示执行高吞吐候选召回，以缩小后续异常评分所需处理的样本范围；在候选集合上执行张量化异常评分，输出异常分数或排序结果；最后结合日志、统计指标与告警规则完成结果输出与追踪，如图~\ref{fig:fin-ad-standard-flow} 所示\citep{zhao2022tod,xiao2025flashanns}。也就是说，本文所讨论的问题并不是某一个评分算法的局部优化，而是围绕``向量化表示---候选召回---异常评分---结果输出与追踪''这一完整处理链路展开。

围绕这一系统处理流程，本文选择 \textbf{PyTOD--FlashANNS 融合异常检测系统} 作为研究对象。PyTOD 的优势在于将异常检测过程抽象为一组可复用的张量算子，使距离计算、排序、聚合与统计过程能够在 GPU 上以统一语义执行，从而承担异常评分主干；FlashANNS 的优势则在于为大规模候选召回提供异步 I/O---计算流水线和高吞吐检索能力，从而承担候选召回前端，如图~\ref{fig:pytod-flashanns-concept} 所示\citep{zhao2022tod,xiao2025flashanns}。然而，二者在功能上的互补并不意味着简单串联即可自动得到高性能系统。若查询特征在 CPU 上构造，再传输至 GPU 进行候选检索，而候选结果又返回 CPU 重新组织后再送回 GPU 评分，则系统瓶颈将从算子执行转移到 CPU$\leftrightarrow$GPU 数据搬运与阶段同步上\citep{faiss_gpu_wiki,rapids_rmm_pinned}；进一步地，在多 GPU 场景中，若采用索引分片并将多个 GPU 上的局部结果集中到 CPU 归并，则 CPU 还可能重新成为系统瓶颈\citep{cuvs_multi_gpu_ivf_host}。
...
围绕上述研究对象与研究问题，可以进一步看到，面向金融场景的大规模异常检测系统，其性能瓶颈并不来自单一环节，而是由异常评分复杂度、候选召回开销、数据路径组织成本以及多 GPU 扩展代价共同决定。为明确本文后续研究重点，有必要对这些核心问题作进一步分析。

\subsection{本文关注的核心系统问题}

围绕上述系统对象，本文研究内容主要聚焦于以下几个彼此关联的核心问题。
...
基于上述分析可以看到，本文真正要解决的问题，并不是提出一种新的异常检测理论模型，而是围绕金融大规模异常检测任务，研究如何将候选召回、异常评分、数据路径优化和多 GPU 扩展统一到一个可部署、可扩展、可验证的系统框架中。换言之，本文的研究主线在于：以前端候选召回压缩评分范围，以 PyTOD 承担张量化异常评分主干，以 GPU 主导执行链路压缩 CPU$\leftrightarrow$GPU 数据路径，并在此基础上进一步讨论多 GPU 条件下的数据并行、结果聚合与共享 I/O 约束。基于这一研究边界与主线，下一节将进一步给出本文的研究目标与技术路线。
